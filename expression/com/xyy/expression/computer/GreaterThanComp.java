 ...
 *
 */
public class GreaterThanComp extends TreatComp {

	public GreaterThanComp(Computer context) {
		super(context);
	}
	...
	public void comp(Token token, Context ctx, Stack<OperatorData> stOutput)
		...
		if (token.getTokenType() == TokenType.TOKEN_TYPE_OPE
			...
			OperatorData result = greaterThan(left, right);
			if (result != null) {
	...
	public static OperatorData greaterThan(OperatorData left, OperatorData right) {
		...
		if (DataHelper.isNumber(left) && DataHelper.isNumber(right)) {
			...
			Boolean o = ((ICompare) left.value).gt(right.value, true);
			return new OperatorData(Boolean.class, o);
		} else if (right.value instanceof ICompare) {
			Boolean o = ((ICompare) right.value).gt(left.value, false);
			return new OperatorData(Boolean.class, o);
