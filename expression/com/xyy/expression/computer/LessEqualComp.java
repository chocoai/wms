 ...
 * 
 */
public class LessEqualComp extends TreatComp {

	public LessEqualComp(Computer context) {
		super(context);
	...
	public void comp(Token token, Context ctx, Stack<OperatorData> stOutput)
		...
		if (token.getTokenType() == TokenType.TOKEN_TYPE_OPE
			...
			OperatorData result = lessThan(left, right);
			if (result != null) {
	...
	public static OperatorData lessThan(OperatorData left, OperatorData right) {
		...
		if (DataHelper.isNumber(left) && DataHelper.isNumber(right)) {
			...
			Boolean o = ((ICompare) left.value).le(right.value, true);
			return new OperatorData(Boolean.class, o);
		} else if (right.value instanceof ICompare) {
			Boolean o = ((ICompare) right.value).le(left.value, false);
			return new OperatorData(Boolean.class, o);
			...
			Date t_date = null;
			try {
				t_date = DateFormat.getInstance().parse(right.value.toString());
				return new OperatorData(Boolean.class,
