...
public class PropertiesPlugin implements IPlugin {
	...
	public boolean start() {
		...
		if (db_type.equals(DictKeys.db_type_postgresql)) { // pg 数据库连接信息
			...
			// 读取当前配置数据库连接信息
			paramMap.put(DictKeys.db_connection_driverClass, "com.microsoft.sqlserver.jdbc.SQLServerDriver");
			paramMap.put(DictKeys.db_connection_jdbcUrl, properties.getProperty("sqlserver.jdbcUrl").trim());
			paramMap.put(DictKeys.db_connection_userName, properties.getProperty("sqlserver.userName").trim());
			paramMap.put(DictKeys.db_connection_passWord, properties.getProperty("sqlserver.passWord").trim());

			// 解析数据库连接URL，获取数据库名称
			String jdbcUrl = (String) getParamMapValue(DictKeys.db_connection_jdbcUrl);
			String database = jdbcUrl.substring(jdbcUrl.indexOf("=") + 1);
		...
		// paramMap.put(DictKeys.config_luceneIndex,
		// properties.getProperty(DictKeys.config_luceneIndex).trim());
		...
		paramMap.put(DictKeys.config_passErrorCount_key,
				Integer.valueOf(properties.getProperty(DictKeys.config_passErrorCount_key)));

		paramMap.put(DictKeys.config_passErrorHour_key,
				Integer.valueOf(properties.getProperty(DictKeys.config_passErrorHour_key)));
		...
		while (enumNames.hasMoreElements()) {
			...
			if (!paramMap.containsKey(name)) {
				paramMap.put(name, properties.getProperty(name));
			}
		}
	...
	private void initAppErpRelationDbConfig() {
		// 读取当前配置数据库连接信息
		paramMap.put(DictKeys.sqlserver_db_connection_driverClass, properties.getProperty("sqlserver.driverClass").trim());
		paramMap.put(DictKeys.sqlserver_db_connection_jdbcUrl, properties.getProperty("sqlserver.jdbcUrl").trim());
		paramMap.put(DictKeys.sqlserver_db_connection_userName, properties.getProperty("sqlserver.userName").trim());
		paramMap.put(DictKeys.sqlserver_db_connection_passWord, properties.getProperty("sqlserver.passWord").trim());

		// 解析数据库连接URL，获取数据库名称
		String jdbcUrl = (String) getParamMapValue(DictKeys.sqlserver_db_connection_jdbcUrl);
		String database = jdbcUrl.substring(jdbcUrl.indexOf("=") + 1);
		database = database.substring(0, database.indexOf(";"));
		...
		// 把数据库连接信息写入常用map
		paramMap.put(DictKeys.sqlserver_db_connection_ip, ip);
		paramMap.put(DictKeys.sqlserver_db_connection_port, port);
		paramMap.put(DictKeys.sqlserver_db_connection_dbName, database);
	}
