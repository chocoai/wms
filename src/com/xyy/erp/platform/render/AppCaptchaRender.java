package com.xyy.erp.platform.render;
...
import com.xyy.erp.platform.common.tools.ToolRandoms;
 ...
 * @author
 */
public class AppCaptchaRender extends Render {

	private static Logger log = Logger.getLogger(AppCaptchaRender.class);
	...
	private String graphics(BufferedImage bufferedImage) {
		...
		for (int i = 0; i < 4; i++) {
			...
			// 在图片上绘制系统生成的随机字符
			graphics.drawString(String.valueOf(tmp), 20 * i + 20, 25);
		}
		...
		textLayout.draw(graphics, 30, 50);// 对字体加投影，第二个是左右相距，越大越远，第三个参数是上下两层相距距离，越大越近
