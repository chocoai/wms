window.ERPMultibill = {
    ...
    pageInit: function () {
        ...
        clone(_model, parent.scope._env);
...
//        this.__loadMultiBillData();
    ...
    refCreateTarget:function(ruleType,bill,scope){
        var me = this;
        var req={};
...
//                    if(ret.action=="go_target_bill_editor"){
                    ...
                    if(ruleType=='PUSH'){
                            ...
                            for( var i=0;i<ret.datas._models.length;i++){
                                var tableKey=ret.datas._models[i];
                                // if(!tableKey){continue;}
                                if (ret.datas[tableKey] && !ret.datas[tableKey].head) {
                                    for(var k=0;k<ret.datas[tableKey].cos.length;k++){
                                        scope.insertRows(tableKey,ret.datas[tableKey].cos[k])
                                    }
                                }
                            }
                            window.localStorage.removeItem("targetBill");
