    ...
    workflow.shape.Line.extend(workflow.core.Shape, {
        ...
        __renderBox:function(){
            if(this.drawType==1){
                ...
                //this.canvas.lineTo(this.tp.x,this.tp.y);
                ...
                this.canvas.beginPath();
                this.canvas.moveTo(this.sp.x,this.sp.y);
                //this.__drawBrokenLine(this.cp.x,this.cp.y);
                //this.tp=getNearestPoint(this.cp,this.target.getLinkedLinePointTo());
                this.canvas.quadraticCurveTo(this.cp.x,this.cp.y,this.tp.x,this.tp.y);
                //this.canvas.strokeStyle = "red";
                this.canvas.stroke();
        ...
        toJSON:function() {
            ...
            lineResult.cp=this.cp;
            lineResult.sp=this.sp;
            lineResult.name=this.text;
            lineResult.drawType=this.drawType;

            lineResult.to=this.target.text;
            lineResult.adId=this.source.OID;

            lineResult.cid=this.CID;
            lineResult.oid=this.OID;
            lineResult.sid=this.source.CID;
            lineResult.tid=this.target.CID;

            //lineResult.po=this.po;
            //return  JSON.stringify(result)
            return lineResult;
        },
        fromJSON:function(json,line)
        {
            line.text=json.name;
            line.cp=json.cp;
            line.OID=json.oid;
            line.drawType=json.drawType;
        }
    });
//单据
    /**
    ...
    workflow.shape.Task.extend(workflow.core.Shape, {
        ...
        __renderText:function(){
            ...
            this.canvas.fillText(this.text,this.x+(this.width-this.fontWidth)/2,this.y+30);
            this.canvas.restore();
        },
        toJSON:function() {
            var result={};
