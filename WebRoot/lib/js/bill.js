window.ERPBill = {
    ...
    save: function (type, fn) {
        ...
        if (this._model) {
            ...
            var _save = function (reqParams) {
                ...
                //
                me.$http.post("/bill/data/bill-save", $.param(reqParams),
                    {
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
                        }
            ...
            if (JSON.stringify(postData) != "{}") {
                me.$http.post("/bill/data/rule", $.param({postData: JSON.stringify(postData)}),
                    {
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
                        }
    ...
    saveForWF: function (scopes) {
        ...
        if (this._model) {
            ...
            clone(this._env, reqParams);
            reqParams.model = angular.toJson(this._model);
            this.$http.post("/bill/data/bill-save", $.param({scopeParam: angular.toJson(scopeParam)}),
                {
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
                    }
   ...
   waifuhe: function () {
    ...
    getTask:function (){
         ...
         if (bills.length == 0) {
             ...
             for (var i = 0; i < bills.length; i++) {
                 if (bills[i].status >= 36 || bills[i].status == 26) {
                     alert("该单已经拣货完成！");
                     return;
                 }
             }
         }
         if (window.confirm('确认索取？')) {
         	NProgress.start();
             reqParams.bills = angular.toJson(bills);
             reqParams.status = -1;
             reqParams.billID = bills[0].BillID;
             reqParams.dingdanbianhao = bills[0].dingdanbianhao;
             reqParams.taskCode = bills[0].taskCode;
             console.log(reqParams);
             if(bills[0].status == 32 || bills[0].status == 28){
                 this.$http.post("/spe/route/getTask", $.param(reqParams),
                         {
                             headers: {
                                 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
                             }
                         	 ...
                         	 NProgress.done();
                         if (data.status == 1) {
                        	 window.location.href = '/bill/view/bill?billKey=' + me._env.billkey + '&BillID=' + bills[0].BillID;
                             alert("任务索取成功！");
                         } else {
                             alert("任务索取失败,原因" + data.error);
                         }
    ...
    waveZhixing:function () {
        ...
        if (bills.length == 0) {
            ...
            for (var i = 0; i < bills.length; i++) {
                if (bills[i].status != 20&&bills[i].status != 23) {
                    alert("该单据已经执行过，不能重复执行");
                    return;
                }
            }
        }
        if (window.confirm('确认执行？')) {
            ...
            reqParams.status = -1;
            reqParams.billID = bills[0].BillID;
            this.$http.post("/waveExecute/waveExecute", $.param(reqParams),
                {
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
                    }
    ...
    delete: function () {
        ...
        if (window.confirm('确认删除？')) {
            ...
            reqParams.status = -1;
            reqParams.billID = bills[0].BillID;
            this.$http.post("/bill/data/bill-delete", $.param(reqParams),
                {
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
                    }
