...
public class BillRuKuYanShouPreHandler implements PreSaveEventListener {
	...
	public void execute(PreSaveEvent event) {
		...
		List<Record> shouhuoDetails = Db
				.find("select shuliang,yanshoushuliang,BillDtlID from xyy_wms_bill_rukushouhuo_details where BillID =?",
			...
			for (int i = 0; i < bodys.size(); i++) {
				...
				if (BillDtlID.equals(rukuID)) {
					...
					String findYanshoudanBillDtlID = Db
							.queryStr(
									"select BillDtlID from xyy_wms_bill_rukushouhuo_details where BillDtlID =? and yanshouzhuangtai =? ",
									rukuID, 1);
