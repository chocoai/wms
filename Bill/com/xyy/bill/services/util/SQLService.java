...
public class SQLService {
	...
	private static final Log LOG = Log.getLog(SQLService.class);
	/**
	 * 电商订单拉取方法
	 * @return
	 */
	public void DSDDFun() {
		List<Record> order = Db.use(DictKeys.db_dataSource_wms_mid).find("select   * from tb_order_tmp where STATE = 0 and KFSTATE = 0 ");
		List<Record> dsList = new ArrayList<>();
		for (Record record : order) {
			...
			//验证单据是否已生成
			Record flagRecord = Db.findFirst("select * from xyy_erp_bill_dianshangdingdan "
					+ "where danjubianhao ='"+record.getStr("ORDERNO")+"' limit 1");
			if (flagRecord!=null) {
				//单据已下拉，直接跳过
				switch (flagRecord.getInt("status")) {
				case 21://异常
					Db.use(DictKeys.db_dataSource_wms_mid).update("update tb_order_tmp set STATE = 1,yichangyuanyin = '"+flagRecord.getStr("yichangyuanyin")+"' "
							+ "where ORDERNO='"+record.getStr("ORDERNO")+"' and STATE !=1 ");			
					break;
//				case 22://已下推
//					Db.use(DictKeys.db_dataSource_wms_mid).update("update tb_order_tmp set STATE = 2 "
//							+ "where ORDERNO='"+record.getStr("ORDERNO")+"' and  STATE !=2 ");
//					break;
		...
		Db.use(DictKeys.db_dataSource_wms_mid).batchUpdate("tb_order_tmp","ID", order, 500);
		if (dsList==null||dsList.size()==0) {
			return;
		}
		this.relationSave(dsList);
		
	}
	
	private void relationSave(List<Record> order) {
		...
		for (Record record : order) {
			...
			for (Record record2 : orderDetails) {
				...
				if (spBuffer.toString().contains(record2.getStr("CODE"))) {
					Db.use(DictKeys.db_dataSource_wms_mid).update("update tb_order_tmp set state =1,yichangyuanyin = '明细中商品["+record2.getStr("CODE")+"]有重复'");
					record.set("yichangyuanyin", "明细中商品["+record2.getStr("CODE")+"]有重复");
					record.set("status", 21);
					record.set("mxyc", 1);
					record.set("BillID", UUIDUtil.newUUID());
					Db.save("xyy_erp_bill_dianshangdingdan", record);
					break;
				}
			...
			record.set("yichangyuanyin", errorBuffer.toString());
			if (list!=null && list.size()>0 && orderDetails.size()==list.size()) {
				this.saveAction(new BillContext(),this.buildContext(record , list,"dianshangdingdan","dianshangdingdan_details"),"dianshangdingdan");
			}
	 ...
	 * 拉取WMS销售出库单
	 */
	public void XSCKDFun() {
		List<Record> ckList = Db.use(DictKeys.db_dataSource_wms_mid).find("select   djbh,sum(SL) as sl,MIN(ZXS) as zxs,MIN(ZCQMC) as zcqmc from int_wms_xsck_bill where is_zx = '否' group by DJBH");
		List<Record> xyyList = new ArrayList<>();
		for (Record record : ckList) {
			Record ds = new Record();
			...
			Record ckRecord = Db.findFirst("select * from xyy_erp_bill_xiaoshouchukudan "
					+ "where sjdjbh ='"+record.getStr("djbh")+"' limit 1");
			if (ckRecord!=null) {
		...
		if (xyyList==null||xyyList.size()==0) {
			return;
		}
		this.saveDetail(xyyList);
	 ...
	 * @param xyyList
	 */
	private void saveDetail(List<Record> xyyList) {
		SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
		for (Record record : xyyList) {
			List<Record> ckList = new ArrayList<>();
			List<Record> list = new ArrayList<>();
			String danjubianhao = record.getStr("sjdjbh");
			...
			List<Record> xList = Db.find("select * from xyy_erp_bill_xiaoshoudingdan_details "
					+ "where danjubianhao = '"+record.getStr("sjdjbh")+"'  ");
			...
			for (Record record2 : orderDetails) {
				...
				Record oRecord = Db.findFirst("select * from xyy_erp_bill_xiaoshoudingdan_details "
						+ "where shangpinbianhao = '"+sp.getStr("shangpinbianhao")+"' and danjubianhao = '"+record.getStr("sjdjbh")+"'  limit 1");
				if (oRecord==null) {
					LOG.error("销售订单明细不存在【"+record.getStr("sjdjbh")+"】");
					continue;
				}
				...
				DSDetail.set("shangpinmingcheng", sp.getStr("shangpinmingcheng"));
				DSDetail.set("kehubianhao", record.getStr("kehubianhao"));
				DSDetail.set("guige", sp.getStr("guige"));
				...
				DSDetail.set("danjubianhao", record.getStr("danjubianhao"));
				DSDetail.set("orgId", orgId);
				DSDetail.set("orgCode", orgCode);
				DSDetail.set("rowID", UUIDUtil.newUUID());
			...
			if (xSum>0&&chukuSum>0&&xSum==chukuSum) {
				...
				record.set("chengbenjine", chengbenjine);
				record.set("maoli", maoli);
				record.set("oldhanshuijine", oldhanshuijine);
				record.set("youhuijine", youhuijine);
				
				BillContext context = new BillContext();
				//插入打印数据表
				this.printFunc(record,list);
				this.saveAction(context,this.buildContext(record, list, "xiaoshouchukudan", "xiaoshouchukudan_details"),"xiaoshouchukudan");
	...
	private void printFunc(Record record, List<Record> list) {
		...
		head.set("billcode", record.getStr("danjubianhao"));
		...
		Record saveRecord = Db.use(DictKeys.db_dataSource_print_mid).findFirst("select * from xsddmt where billcode = '"+record.getStr("danjubianhao")+"'");
		//插入体
		...
		for (Record sp : list) {
			...
			detail.set("danjia", sp.getBigDecimal("hanshuijia"));
			detail.set("jine", sp.getBigDecimal("hanshuijine"));
	 ...
	 * 拉取WMS销售销售退回入库单
	 */
	public void XSTHRKFun() {
		List<Record> ckList = Db.use(DictKeys.db_dataSource_wms_mid).find("select    djbh,count(*) from int_wms_xsth_bill where is_zx = '否' group by DJBH");
		List<Record> xyyList = new ArrayList<>();
		for (Record record : ckList) {
			Record ds = new Record();
			...
			Record ckRecord = Db.findFirst("select * from xyy_erp_bill_xiaoshoutuihuirukudan "
					+ "where sjdjbh ='"+record.getStr("djbh")+"' limit 1");
			if (ckRecord!=null) {
				Db.use(DictKeys.db_dataSource_wms_mid).update("update int_wms_xsck_bill set is_zx = '是' where djbh = '"+record.getStr("djbh")+"' ");
				continue;
	 ...
	 * @param xyyList
	 */
	private void saveDetail2(List<Record> xyyList) {
		SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
		for (Record record : xyyList) {
			List<Record> ckList = new ArrayList<>();
			List<Record> list = new ArrayList<>();
			...
			for (Record record2 : orderDetails) {
				...
				Record oRecord = Db.findFirst("select * from xyy_erp_bill_xiaoshoutuihuidan_details "
						+ "where shangpinbianhao = '"+sp.getStr("shangpinbianhao")+"' and danjubianhao = '"+record.getStr("sjdjbh")+"'  limit 1");
				if (oRecord==null) {
					LOG.error("销售退回单明细不存在【"+record.getStr("sjdjbh")+"】");
					continue;
				}
			...
			if (xSum>0&&rukuSum>0&&xSum==rukuSum) {
				...
				BillContext context = new BillContext();
				this.saveAction(context,this.buildContext(record, list, "xiaoshoutuihuirukudan", "xiaoshoutuihuirukudan_details"),"xiaoshoutuihuirukudan");
...
//					+ "where a.userId = '"+head.getStr("kpyID")+"' and a.deptId = b.id limit 1");
//			if (dept!=null) {
...
//			}
	...
	//推销售订单到中间库
	public void XSDDFun() {
		List<Record> heads = Db.find("select * from xyy_erp_bill_xiaoshoudingdan where status = 20 and shifouzhifu = 1 and shifouxiatui = 0 ");
		for (Record head : heads) {
			List<Record> bodys = Db.find("select * from xyy_erp_bill_xiaoshoudingdan_details "
					+ "where BillID = '"+head.getStr("BillID")+"'");
			this.XSDDFun(head, bodys);
			//测试用，手动新建销售出库单
//			WMSTempService wService = new WMSTempService();
//			wService.createXSCKD(head,bodys);
		}
	}
	...
	//推销售退回单到中间库
	public void XSTHFun() {
		List<Record> heads = Db.find("select * from xyy_erp_bill_xiaoshoutuihuidan where status = 40 and shifouxiatui = 0 ");
		for (Record head : heads) {
			List<Record> bodys = Db.find("select * from xyy_erp_bill_xiaoshoutuihuidan_details "
					+ "where BillID = '"+head.getStr("BillID")+"'");
			this.XSTHFun(head, bodys);
			//测试用，手动新建销售退回入库单
//			WMSTempService wService = new WMSTempService();
//			wService.createXSTHRKD(head, bodys);
		}
		
	}
	
	//扫描电商订单，进行拦截下推
	public void checkDSDDFun() {
		List<Record> heads = Db.find("select * from xyy_erp_bill_dianshangdingdan where status = 20");
		for (Record head : heads) {
			List<Record> bodys = Db.find("select * from xyy_erp_bill_dianshangdingdan_details "
					+ "where BillID = '"+head.getStr("BillID")+"'");
			this.checkDSDD(head,bodys);
		}
