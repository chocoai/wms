...
public class QueryCaiGouDetailService extends BillBizService {
	...
	public Page<Record> findBizData(BillContext billContext) {
		...
		JSONObject page = billContext.getJSONObject("model").getJSONObject("querycaigoudetail").getJSONObject("page");
	...
	private String buildSQL(BillContext billContext) {
		...
		JSONObject params = billContext.getJSONObject("model").getJSONObject("querycaigoudetail").getJSONObject("params");
		...
		sql = new StringBuffer("SELECT r.* FROM xyy_erp_bill_migratejinxiangkaipiao_details r,xyy_erp_bill_migratejinxiangkaipiao h where 1=1 and r.danjubianhao=h.danjubianhao");
